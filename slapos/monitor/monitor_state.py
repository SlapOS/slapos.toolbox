...
def quickAppendToJsonFile(file_path, content, step_back=2):
  append_mode = 'r+'
  mode = append_mode
  if step_back < 0 or not os.path.exists(file_path) or \
      os.stat(file_path).st_size < step_back:
    # do not try to append to file which doesn't exist or is empty
    mode = 'w'
  with open (file_path, mode) as f:
    if mode == append_mode:
      ...
      f.seek(position)
      f.write('%s}' % ',{}]'.format(json.dumps(content)))
    else:
...
class MonitorStateBuilder(object):
  ...
  def __init__(self, monitor_conf_file):
    ...
    self.documents_folder = os.path.join(self.private_folder, 'documents')
    ...
    self.tmp_dir = os.path.join(
      config.get('promises', 'partition-folder'),
      'tmp'
    )
  ...
  def buildMonitorState(self):
    ...
    with open(self.global_state_file, 'w') as fg:
      fg.write(json.dumps(global_state_dict))

    with open(self.public_state_file, 'w') as fpg:
    ...
    self.writeDocumentList(self.documents_folder)

    self.saveStatisticsData(
      os.path.join(self.documents_folder, 'monitor_state.data.json'),
      global_state_dict
    )
