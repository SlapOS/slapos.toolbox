...
class TestCheckServiceState(TestPromisePluginMixin):
  ...
  def setUp(self):
    ...
    self.process_state_file = os.path.join(self.partition_dir, '.slapgrid/state', '%s.json' % self.script_name)
    ...
    self.second_process_state_file = os.path.join(self.partition_dir, '.slapgrid/state', '%s.json' % self.second_script_name)
    ...
    self.promise_content = textwrap.dedent(
      ...
      """) % {'state-dir': os.path.dirname(self.process_state_file),
              'script-dir': os.path.dirname(self.script_path)}
  ...
  def test_check_process_state_fail(self):
    ...
    self.assertEqual(
      result['result']['message'],
      "Process '%s' script exited with unexpected exit code" % self.script_name
    )
  ...
  def test_check_process_state_multiple_process_all_fail(self):
    ...
    self.assertEqual(
      result['result']['message'],
      """Process '%s' script exited with unexpected exit code
Process '%s' script exited with unexpected exit code
""".strip() % (self.second_script_name, self.script_name)
    )
