...
class DNSResolverTestCase(unittest.TestCase):
  ...
  def test(self):
    input_1 = self._fillInput('1.example.com non.example.com')
    input_2 = self._fillInput('2.example.com 123.124.125.126')
    output = os.path.join(self.working_directory, 'output')

    self._query_answer = {
     '1.example.com': [Answer('127.0.0.1')],
     '2.example.com': [Answer('127.0.0.2')]
    }

    runner = CliRunner()
    result = runner.invoke(dnsresolver.main, ['-o', output, input_1, input_2])
    ...
    with open(output, 'rb') as fh:
      self.assertEqual(
        json.load(fh),
        ['123.124.125.126', '127.0.0.1', '127.0.0.2']
      )
