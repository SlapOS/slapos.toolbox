  ...
  def senseDnsQuery(self):
    ...
    for entry in entry_list:
      ...
      if ip_set != response_ip_set:
        appendError("%s: ERROR resolver %s returned unexpected set of IPs %s, expected %s" % (key, entry['resolver_ip'], ','.join(response_ip_set), ','.join(ip_set)))
        error = True
    if error:
      return
    self.appendInfoMessage("%s: OK all resolvers returnted expected set of IPs %s" % (','.join(ip_set)))
