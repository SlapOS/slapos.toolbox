...
class RunPromise(GenericPromise):
  ...
  def senseDnsQuery(self):
    ...
    for entry in entry_list:
      ...
      if ip_set != response_ip_set:
        if len(response_ip_set) == 0:
          appendError("resolver %s returned no IPs, expected %s debug %s" % (entry['resolver_ip'], ','.join(ip_set)), response_ip_set)
        else:
    ...
    if error:
      return
    self.appendInfoMessage("%s: OK all resolvers returnted expected set of IPs %s" % (key, ', '.join(sorted(ip_set))))
