...
class RunPromise(GenericPromise):
  ...
  def senseTcpServer(self):
    ...
    entry_list = [q for q in self.surykatka_json[key]
      if q['domain'] in [r.strip() for r in hostname.split(',')] and q['port'] == port]
    if len(entry_list) == 0:
      appendError('No data')
      return
    db_ip_list = []
    ...
    if set(db_ip_list) != ip_set:
      appendError("IP list %s differes from expected %s" % (' '.join(sorted(db_ip_list)), ' '.join(sorted(ip_set))))
      error = True
    if error:
      return
