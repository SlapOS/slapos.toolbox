...
class RunPromise(GenericPromise):
  ...
  def sense(self):
    """
    The current implementation tries to do its best to check whether the system
    is up-to-date, mainly to ensure all security fixes are applied. For lack
    of simple & reliable way to check that, this method does several
    independant checks and if they're all positive, it should result in a
    secure system.
    """
    os.environ.pop('LANG', None)
    # To simplify the anomaly() method, we'll issue a single log event.
    error_list = []
    error = error_list.append
    @contextmanager
    def exc():
      """
      Use to wrap a single check. Any failure, including a bug in the promise
      itself, won't prevent doing the next checks.
      """
      try:
        yield
    ...
    with exc():
      ...
      try:
        ...
        os.close(fd)
    args = ['apt-cache', 'policy'] # check apt_preferences(5)
    with exc(): # global check
      # Wrong configuration could lead an upgrade to switch to some backport
      # repository or a non-official one, then the security repository would
      # not be used anymore, and security fixes are likely to be applied with
      # a greater delay or even never.
    ...
    with exc(): # check specific packages
      # This step is mainly used for packages like ungoogled-chromium, coming
      # from a PPA, because we currently have to patch unattended-upgrades in
      # order to upgrade it automatically while other packages other the same
      # repository are not touched.
      # For this last check, the sense method may run while an upgrade is
      # ongoing and we don't want to report a failure on the first negative
      # result.
  ...
  def anomaly(self):
    # For the last check in the sense method, we'll need more than 1 result
    # to see if there's actually some progress or not.
