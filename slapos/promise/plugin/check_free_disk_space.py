...
class RunPromise(GenericPromise):
  ...
  def sense(self):
    ...
    free_space = self.getFreeSpace(disk_partition, db_path, currentdate,
                                   currenttime)
    days_until_full_tuple = self.getDaysUntilFull(disk_partition, db_path, currentdate, currenttime, threshold_days)
    if days_until_full_tuple is not None:
      ...
      if days_until_full < threshold_days:
        self.logger.error("Disk will become full in %.2f days (threshold: %.2f days), checked from %s to %s, %.2f days span" % (
