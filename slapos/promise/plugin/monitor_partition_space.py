...
class RunPromise(GenericPromise):
  ...
  def getAnomaly(self, disk_partition, db_path, user, date, time):
    ...
    try:
      ...
      result = database.select(
        "folder",
        columns = "%s-disk_used*1024, datetime(date || ' ' || time)" % disk_size,
      ...
      last_date = datetime.datetime.strptime(result[-1][1], "%Y-%m-%d %H:%M:%S")
      ...
      # threshold is set at 10% of the disk size
      threshold = 0.10*disk_size
      # use a 1-day window
      ...
      data = pd.concat([x, y, rolling_mad, rolling_mad_upper, rolling_mad_lower], axis=1)
      data.columns = ["date", "free", "mad", "upper_mad", "lower_mad"]
      ...
      self.logger.info("There were %s anomalies in the last 15 days " \
        "(out of %s data, 1 data per %s minutes, threshold: %.2f G)" % (
        len(data[data['is_anomaly'] == True]), len(df), freq, threshold/(1024*1024*1024)))
      return data
    except sqlite3.OperationalError as e:
      # if database is still locked after timeout expiration (another process is using it)
      # we print warning message and try the promise at next run until max warn count
  ...
  def sense(self):
    ...
    self.logger.debug("Partition: %s." % user)
    ...
    if check_date:
      ...
      disk_partition = self.getConfig('test-disk-partition', 'slapuser00')
    else:
      # get last minute
    ...
    if last_data.is_anomaly:
      self.logger.error("Anomaly detected on %s. Free disk space at this time: %s G." % (
        last_date, last_data.free/(1024*1024*1024)))
    else:
